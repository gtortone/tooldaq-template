...
bool SlowControl::Initialise(std::string configfile, DataModel &data){
  ...
  if(!m_variables.Get("slow_control_port", slow_control_port))
     slow_control_port = "22222";
               ...
               *m_log<<orange << iss.str() << plain << std::endl << std::endl;
               ...
               if(type == "Request") {
                  ...
                  *m_log << yellow << msg_str << plain << std::endl << std::endl;
                  ...
                  zmq::message_t config_msg(msg_str.length()+1);

                  snprintf ((char *) config_msg.data(), msg_str.length()+1 , "%s" , msg_str.c_str());

                  slow_control_sock->send(config_msg);
               }
            }     // end if recv(&msg)
         }        // end if recv(&blank)
      }           // end if recv(&identity)
   }              // end if revents
